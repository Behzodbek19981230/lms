...
export class TelegramNotificationService {
   ...
   */
  private async getCenterMainChannel(
    centerId: number,
  ...
  async sendAttendanceSummaryToCenterChannel(params: {
    ...
    if (params.absentStudents && params.absentStudents.length > 0) {
      ...
      messageLines.push('');
      messageLines.push(
        `❌ <b>Kelmadi ro‘yxati (${params.absentStudents.length}):</b>`,
      );
      messageLines.push(list);
      if (params.absentStudents.length > 25) {
        messageLines.push(
          `... va yana ${params.absentStudents.length - 25} ta`,
        );
      }
    }
  ...
  async sendDuePaymentsReminderToCenterChannel(params: {
    ...
    for (const [groupName, rows] of byGroup.entries()) {
      ...
      rows.forEach((r, idx) => {
        const amount =
          typeof r.amount === 'number' ? String(r.amount) : r.amount;
        const desc = r.description ? ` — ${r.description}` : '';
        groupBlockLines.push(
          `${idx + 1}. ${r.studentName} — ${amount} so‘m${desc}`,
        );
      });
