...
export class UsersService {
  ...
  async findAll(centerId?: string, role?: string, includeGroups?: boolean, includeSubjects?: boolean, unassigned?: boolean): Promise<User[]> {
    const queryBuilder = this.userRepository.createQueryBuilder('user')
      .leftJoinAndSelect('user.center', 'center')
      .leftJoinAndSelect('user.subjects', 'subjects');
    
    if (unassigned) {
      queryBuilder.where('user.center IS NULL')
        .andWhere('user.role != :superadmin', { superadmin: UserRole.SUPERADMIN });
    } else if (centerId) {
      queryBuilder.where('center.id = :centerId', { centerId: Number(centerId) });
    }
  ...
  async connectTelegram(userId: number, telegramUsername: string) {
    ...
    return {
      success: true,
      message: 'Telegram ulanish so\'rovi yuborildi. Iltimos, bot xabarini tasdiqlang.',
      telegramUsername,
    };
  }
}
