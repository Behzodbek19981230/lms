...
export class PdfService {
  ...
  async generateExamVariantPDF(
    ...
    this.logger.log(`Generating PDF for exam variant ${variant.id}`);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.on('end', () => {
          ...
          this.logger.log(`PDF created successfully. Size: ${pdfBuffer.length} bytes`);
          resolve(pdfBuffer);
        });
        doc.on('error', (error) => {
          this.logger.error('PDF creation error:', error);
          reject(new InternalServerErrorException(`PDF yaratishda xatolik: ${error.message}`));
        });
        ...
        reject(new InternalServerErrorException(`PDF generatsiya xatolik: ${error.message}`));
      }
    });
  }
  ...
  private addQuestionsToPDF(
    ...
    questions.forEach((question, index) => {
      ...
      yPosition += this.calculateTextHeight(doc, questionText, pageWidth - 20, 11) + 15;

      // Rasm bo'lsa
      if (question.imageBase64) {
        doc.fontSize(9).font('Helvetica')
          .text('[Rasm biriktirilgan]', 70, yPosition, { width: pageWidth - 20 });
        yPosition += 20;
      }
      ...
      if (question.answers && question.answers.length > 0) {
        question.answers.forEach((answer, answerIndex) => {
          ...
          yPosition += this.calculateTextHeight(doc, answerText, pageWidth - 40, 10) + 8;
        });
        ...
        const trueText = options.isAnswerKey && question.answers?.find(a => a.isCorrect && a.text.toLowerCase().includes('to\'g\'ri')) 
          ? 'A) To\'g\'ri ✓' : 'A) To\'g\'ri';
        const falseText = options.isAnswerKey && question.answers?.find(a => a.isCorrect && a.text.toLowerCase().includes('noto\'g\'ri')) 
          ? 'B) Noto\'g\'ri ✓' : 'B) Noto\'g\'ri';
  ...
  private createExamVariantPDF(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (variant.student) {
          doc.fontSize(12).font('Helvetica')
            .text(`Talaba: ${variant.student.firstName} ${variant.student.lastName}`, 50, 130, { 
              align: 'center', 
              width: pageWidth 
            });
        }
        ...
        // Chiziq
        doc.moveTo(50, 190).lineTo(doc.page.width - 50, 190).stroke();
        ...
        if (variant.questions && variant.questions.length > 0) {
          ...
          const questions = variant.questions.map(vq => ({
            ...
            imageBase64: null,
            metadata: null,
            answers: vq.options ? vq.options.map((opt: string, idx: number) => ({
              id: idx,
              text: opt,
   ...
   */
  async generatePDFFromHTML(html: string, title: string = 'Document'): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fontSize(18).font('Helvetica-Bold').text(pageTitle, { align: 'center' });
        doc.moveDown(2);
