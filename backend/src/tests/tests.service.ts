...
export class TestsService {
  ...
  async remove(
    ...
    // Best-effort cleanup for stable printable HTML files
    // (generated by POST /tests/:id/printable-html)
    try {
      const uploadsDir = this.getUploadsDir();
      const weeklyDir = join(uploadsDir, 'weekly-tests');
      await Promise.all([
        fs
          .unlink(join(weeklyDir, `weekly-test-${id}.html`))
          .catch(() => undefined),
        fs
          .unlink(join(weeklyDir, `weekly-test-${id}-answers.html`))
          .catch(() => undefined),
      ]);
    } catch {
  ...
  private getUploadsDir(): string {
    // Mirror logic in TestGeneratorService to keep paths consistent.
    const candidates = [
