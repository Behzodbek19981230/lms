...
export default function CenterTeachersPage() {
	...
	useEffect(() => {
		setPage(1);
	}, [searchTerm, statusFilter, teachers.length]);

	const totalPages = Math.max(1, Math.ceil(filteredTeachers.length / pageSize));
	const safePage = Math.min(page, totalPages);
	const pageStart = (safePage - 1) * pageSize;
	const pageEnd = Math.min(pageStart + pageSize, filteredTeachers.length);
	const paginatedTeachers = filteredTeachers.slice(pageStart, pageEnd);
	...
	const handleStatusToggle = async (teacherId: number, currentStatus: boolean) => {
		try {
			...
			setTeachers((prev) =>
				prev.map((teacher) => (teacher.id === teacherId ? { ...teacher, isActive: !currentStatus } : teacher))
			);
	...
	const handleExport = async () => {
		try {
			...
			const rows = teachers.map((teacher) => ({
				Ism: teacher.firstName,
				Familiya: teacher.lastName,
				Username: teacher.username,
				Telefon: teacher.phone || '',
				Holat: teacher.isActive ? 'Faol' : 'Nofaol',
				Fanlar: teacher.subjects?.map((s) => s.name).join('; ') || '',
				'Guruhlar soni': teacher.groups?.length || 0,
				"Ro'yxatdan o'tgan sana": new Date(teacher.createdAt).toLocaleDateString(),
			}));
			...
			const fileName = `teachers-${ts.getFullYear()}-${pad(ts.getMonth() + 1)}-${pad(ts.getDate())}.xlsx`;

			XLSX.writeFile(wb, fileName, { bookType: 'xlsx' });
		} catch (error: any) {
			toast({
				title: 'Xato',
				description: 'Excel export qilishda xatolik',
				variant: 'destructive',
			});
		}
	};
	...
	const handleEditSubmit = async () => {
		try {
			...
			setTeachers((prev) =>
				prev.map((teacher) => (teacher.id === editForm.id ? { ...teacher, ...editForm } : teacher))
			);
			...
			<Dialog open={showDetails} onOpenChange={setShowDetails}>
				<DialogContent className='max-w-4xl'>
					<DialogHeader>
						...
						<DialogDescription>
							{selectedTeacher && `${selectedTeacher.firstName} ${selectedTeacher.lastName}`} haqida
							batafsil ma'lumot
						</DialogDescription>
					</DialogHeader>
							...
							<div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
								...
								<div>
									...
									<p className='text-sm text-muted-foreground'>
										{selectedTeacher.phone || "Ko'rsatilmagan"}
									</p>
								</div>
								<div>
									...
									<div className='mt-2 space-y-2'>
										{selectedTeacher.groups.map((group) => (
											...
											>
												...
												<Badge variant='secondary'>{group.studentsCount} student</Badge>
											</div>
										))}
									</div>
								</div>
			...
			<Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
				<DialogContent className='sm:max-w-[425px]'>
					<DialogHeader>
						...
						<DialogDescription>O'qituvchi uchun barcha ma'lumotlarni kiriting.</DialogDescription>
					</DialogHeader>
			...
			<Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
				<DialogContent className='sm:max-w-[425px]'>
					<DialogHeader>
						...
						<DialogDescription>O'qituvchi ma'lumotlarini o'zgartirish.</DialogDescription>
					</DialogHeader>
					<div className='grid gap-4 py-4'>
						<div className='grid grid-cols-4 items-center gap-4'>
