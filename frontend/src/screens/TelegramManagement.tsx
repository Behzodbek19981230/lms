...
import TelegramMessageMonitor from '@/components/telegram/TelegramMessageMonitor'; // ✅ NEW
...
const TelegramManagement: React.FC = () => {
	...
	const [groups, setGroups] = useState<Group[]>([]); // ✅ NEW
	...
	const fetchData = async () => {
		try {
			...
			// Fetch all chats, available tests, centers, subjects, groups, users, and unlinked users
			const [
				chatsResponse,
				testsResponse,
				centersResponse,
				...
				request.get('/groups'), // ✅ NEW: All groups
			...
			setGroups(groupsResponse.data || []); // ✅ NEW
	...
	const handleRegisterChat = async () => {
		...
		try {
			...
			const chatData = {
				...
				groupId: newChat.groupId ? parseInt(newChat.groupId) : undefined, // ✅ NEW
			};
	...
	const generateChannelName = () => {
		...
		const group = groups.find((g) => g.id.toString() === newChat.groupId); // ✅ NEW

		if (center && subject && group) {
			// ✅ NEW: Include group in name
			...
			return `@universal_${centerName}_${subjectName}_${groupName}`;
		} else if (center && subject) {
			const centerName = center.name.toLowerCase().replace(/\s+/g, '');
			const subjectName = subject.name.toLowerCase().replace(/\s+/g, '');
	...
	return (
		<div className='space-y-4 sm:space-y-6 p-3 sm:p-4'>
			<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4'>
				<h1 className='text-xl sm:text-2xl md:text-3xl font-bold'>Telegram Integratsiyasi</h1>
				...
				<CardContent className='space-y-4'>
					<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
						<div>
							...
							/>
							<p className='text-[10px] sm:text-xs md:text-sm text-muted-foreground mt-1 leading-relaxed'>
								<span className='font-semibold'>Eslatma:</span> Chat ID olish uchun botni admin qiling
								va{' '}
								<a
						...
						<div>
							...
							<Select
								...
								onValueChange={(value) =>
									setNewChat({ ...newChat, subjectId: value || '', groupId: '' })
								}
							>
								<SelectTrigger>
						...
						<div>
							...
							<p className='text-xs text-muted-foreground mt-1'>
								💡 Har bir guruhga alohida kanal biriktirish tavsiya etiladi
							</p>
						</div>
					</div>
					...
					{newChat.centerId && newChat.subjectId && (
						<div className='p-3 bg-blue-50 border border-blue-200 rounded-md'>
							<p className='text-xs md:text-sm text-blue-700'>
								<strong className='text-xs md:text-sm'>Tavsiya etilgan kanal nomi:</strong>{' '}
								{generateChannelName()}
							</p>
						</div>
					)}
				...
				<CardContent className='space-y-4'>
					{unlinkedUsers.length === 0 ? (
						<p className='text-xs md:text-sm text-gray-500 text-center py-4'>
							Bog'lanmagan Telegram foydalanuvchilari yo'q. Foydalanuvchilar avval botni ishga
							tushirishlari kerak.
						</p>
					) : (
						<>
										...
										<SelectContent>
											{unlinkedUsers.map((user) => (
												<SelectItem key={user.id} value={user.telegramUserId}>
													...
													{user.telegramUsername || 'no_username'})
												</SelectItem>
											))}
										</SelectContent>
									</Select>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleSendTest} disabled={loading || !selectedTest || !selectedChannel}>
			...
			<Card>
				...
				<CardContent>
					{filteredChats.length === 0 ? (
						<p className='text-xs md:text-sm text-gray-500 text-center py-4'>
							Hech qanday Telegram chat hali ro'yxatga olinmagan.
						</p>
					) : (
						<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
							{filteredChats.map((chat) => (
								<div key={chat.id} className='relative border rounded-lg p-4 space-y-2'>
									...
									>
										...
										>
											<path
												...
												strokeWidth={2}
												d='M6 7h12M9 7V5a3 3 0 013-3v0a3 3 0 013 3v2m-7 0h8m-8 0a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V9a2 2 0 00-2-2H6z'
											/>
										</svg>
									</button>
									...
									<p className='text-xs md:text-sm text-gray-500'>ID: {chat.chatId}</p>
									{chat.centerName && (
										<p className='text-xs md:text-sm text-gray-600'>Markaz: {chat.centerName}</p>
									)}
									{chat.subjectName && (
										<p className='text-xs md:text-sm text-gray-600'>Fan: {chat.subjectName}</p>
									)}
									{/* ✅ NEW: Show group name */}
									{chat.groupName && (
										<p className='text-xs md:text-sm text-gray-600 font-semibold'>
											👥 Guruh: {chat.groupName}
										</p>
									)}
									{chat.lastActivity && (
			...
			{/* ✅ NEW: Message Queue Monitoring */}
			<div className='mt-8'>
				<h2 className='text-xl sm:text-2xl font-bold mb-4'>📊 Xabar Yuborilishi Monitoring</h2>
					...
					<div className='space-y-3'>
						...
						<div>
							...
							<ul className='list-disc list-inside text-xs md:text-sm text-gray-600 space-y-1'>
								...
								<li>
									Savollarga quyidagi formatda javob bering:{' '}
									<code className='bg-gray-100 px-1 rounded'>#T123Q1 A</code>
								</li>
