...
const StudentDashboard = () => {
	...
	const enrolledCourses = subjects.slice(0, 3).map((subject, index) => {
		...
		const completedExams = subjectExams.filter(
			(exam) => exam.status === 'completed' || exam.status === 'submitted'
		).length;
	...
	return (
		<div className='min-h-screen bg-gradient-to-br from-background via-background/95 to-primary/5'>
					...
					>
						<Button variant='outline' size='sm' className='flex-1 sm:flex-initial'>
							...
							{notifications.length > 0 && (
								<Badge className='ml-1 sm:ml-2 bg-destructive text-[10px] sm:text-xs px-1.5'>
			...
			<div className='p-3 sm:p-4 md:p-6'>
				{/* Stats Grid */}
				<div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8'>
					<Card className='border-border'>
						...
						<CardContent>
							...
							<p className='text-[10px] sm:text-xs text-accent'>Faol</p>
						</CardContent>
					</Card>
					...
					<Card className='border-border'>
						<CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
							<CardTitle className='text-sm font-medium text-muted-foreground'>To'lovlar</CardTitle>
					...
					<Card className='border-border'>
						<CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
							<CardTitle className='text-sm font-medium text-muted-foreground'>Mukofotlar</CardTitle>
							<Award className='h-4 w-4 text-primary' />
						</CardHeader>
						<CardContent>
							<div className='text-2xl font-bold text-foreground'>
								{grades.filter((g) => g.percentage >= 80).length || 0}
							</div>
				...
				<Card className='mt-6 border-border'>
					...
					<CardContent>
						<div className='space-y-3'>
							{examVariants
								.filter((variant) => variant.status === 'generated' || variant.status === 'started')
								.map((variant) => (
									<div
										key={variant.id}
										className='flex items-center justify-between p-3 bg-muted rounded-lg'
										...
										<div className='flex items-center space-x-3'>
											...
											<div>
												<p className='text-sm text-foreground font-medium'>
													Imtihon varianti #{variant.variantNumber}
												</p>
												<p className='text-xs text-muted-foreground'>
												...
												{variant.startedAt && (
													<p className='text-xs text-muted-foreground'>
														Boshlangan:{' '}
														{moment(variant.startedAt).format('DD.MM.YYYY HH:mm')}
													</p>
												)}
											</div>
										</div>
							...
							{examVariants.filter(
								(variant) => variant.status === 'generated' || variant.status === 'started'
							).length === 0 && (
								...
								>
									<div className='flex items-center space-x-3'>
										<div className='p-2 bg-primary/10 rounded-full'>
											...
											{!['exam', 'test', 'grade'].includes(notification.type) && (
